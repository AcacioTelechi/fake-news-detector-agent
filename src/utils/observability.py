    ...
    def on_llm_end(self, response: LLMResult, *, run_id, parent_run_id=None, **kwargs):
        ...
        for gen_list in response.generations:
            for gen in gen_list:
                ...
                elif hasattr(msg, "response_metadata") and msg.response_metadata and total_tokens == 0:
...
def track_node_metrics(node_name: str):
    ...
    def decorator(func: Callable[[AgentState, Runtime[AgentContext]], Dict[str, Any]]):
        @wraps(func)
        def wrapper(state: AgentState, runtime: Runtime[AgentContext]) -> Dict[str, Any]:
...
def print_metrics_summary(state: AgentState):
    """
    ...
    for node_name, node_metrics in metrics.items():
        ...
        print(f"  Tokens: {node_metrics.total_tokens} "
    ...
    print(f"  Tokens totais: {total_tokens} ({total_prompt_tokens} prompt + {total_completion_tokens} completion)")
    print("="*60 + "\n")
